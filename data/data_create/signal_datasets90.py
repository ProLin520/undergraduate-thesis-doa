...
ext_lib_base = root / "Graduation" / "external" / "DOA_est_Master-master"

print(f"正在检查外部库路径: {ext_lib_base}")
target_file = ext_lib_base / "utils" / "batch_matrix_operator.py"

if target_file.exists():
    spec = importlib.util.spec_from_file_location("batch_matrix_operator", str(target_file))
    bmo = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(bmo)
    globals().update({k: v for k, v in vars(bmo).items() if not k.startswith('_')})
    print(" 已通过物理路径强制加载 batch_matrix_operator")
else:
    print(f" 找不到物理文件: {target_file}")
    print(f"该目录下实际内容有: {list(ext_lib_base.glob('*'))}")

if str(ext_lib_base) not in sys.path:
    sys.path.insert(0, str(ext_lib_base))
...
class ULA_dataset:
        ...
        for attr_name in dir(self):
            ...
            if isinstance(attr_value, list) and len(attr_value) > 0:
                try:
                    ...
                    # 🌟 关键修复：如果形状不一致（如snap可变），则转为对象数组 (Object Array)
                    result[attr_name] = np.array(attr_value, dtype=object)
    ...
    def get_spatial_sp(self, doas, grid=None):
        ...
        step = grid[1] - grid[0]
        doas_clipped = np.clip(doas, grid[0], grid[-1])
        pos = (doas_clipped - grid[0]) / step

        left_idx = np.floor(pos).astype(np.int64)
        right_idx = np.clip(left_idx + 1, 0, len(grid) - 1)
        frac = pos - left_idx

        same_bin = right_idx == left_idx
        frac[same_bin] = 0.0
        score_left = 1.0 - frac
        score_right = frac

        spatial_sp = np.zeros((doas.shape[0], len(grid)), dtype=np.float32)
        batch_idx = np.arange(doas.shape[0])[:, None]
        spatial_sp[batch_idx, left_idx] += score_left
        spatial_sp[batch_idx, right_idx] += score_right
    ...
    def get_spatial_sp_sparse(self, doas, grid=None):
        ...
        doas_clipped = np.clip(doas, grid[0], grid[-1])
        pos = (doas_clipped - grid[0]) / step
        nearest_idx = np.floor(pos + 0.5).astype(np.int64)
        nearest_idx = np.clip(nearest_idx, 0, len(grid) - 1)

        spatial_sp = np.zeros((doas.shape[0], len(grid)), dtype=np.float32)
        spatial_sp[np.arange(doas.shape[0])[:, None], nearest_idx] = 1.0

        return spatial_sp

    def _self_test_spatial_sp_mapping(self):
        test_doas = np.array([[-90.0, 0.0, 10.0, 89.7]], dtype=np.float32)
        dense_sp = self.get_spatial_sp(test_doas)
        sparse_sp = self.get_spatial_sp_sparse(test_doas)

        dense_bins = [
            (float(self.signal_grid[idx]), float(dense_sp[0, idx]))
            for idx in np.where(dense_sp[0] > 0)[0]
        ]
        sparse_bins = [float(self.signal_grid[idx]) for idx in np.where(sparse_sp[0] > 0)[0]]

        print("Dense spatial_sp non-zero bins:", dense_bins)
        print("Sparse spatial_sp nearest bins:", sparse_bins)
...
if __name__ == '__main__':
    ...
    dataset._self_test_spatial_sp_mapping()
    snap = 100
    snr = 10
    k = 3
