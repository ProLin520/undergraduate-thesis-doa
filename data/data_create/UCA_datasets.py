...
class UCA_dataset:
    def __init__(self,
        ...
        self.d = self.r * np.sqrt(2 * (1 - np.cos(phi[1])))
        self.array_pos_x = self.r * np.cos(phi)
        self.array_pos_y = self.r * np.sin(phi)
        self.array_pos_z = np.zeros_like(self.array_pos_y)
        self.array_pos = np.stack([self.array_pos_x, self.array_pos_y, self.array_pos_z], 0)
        self.signal_grid = np.arange(st_angle, ed_angle + 0.001, step)  # self.st_angle = self.signal_grid[0]

        distance = [np.sqrt(np.sum((self.array_pos[:, i]-self.array_pos[:, 0])**2)) for i in range(M)]
        self.scale_distance = np.array(distance)/distance[1]
    ...
    def save_all_data(self, save_dir):
        """
        将实例的所有列表属性保存到字典中并压缩存储。
        """
        result = {}
        for attr_name in dir(self):
            # 过滤掉方法和特殊属性，只保留列表类型的数据
    ...
    def Create_DOA_data(self, k: int,
        ...
        Args:
            ...
            doas: 所有入射信号对应的角度 / (b,2,k) np.ndarray, direction af arrival of signals
            snrs_db: 所有入射信号对应的信噪比(db) / (b,k) np.ndarray, snr of signals(db)
        ...
        assert doas.shape[-1] == k, 'error signal setting'
        ...
        # build matrix A
        doas = doas / 180 * np.pi
        proj_x = np.cos(doas[:, 0, :]) * np.sin(doas[:, 1, :])  # sin(phi)*cos(theta)
        proj_y = np.sin(doas[:, 0, :]) * np.sin(doas[:, 1, :])  # sin(phi)*sin(theta)
        proj_z = np.cos(doas[:, 1, :])  # cos(phi)
        proj_xyz = np.stack([proj_x, proj_y, proj_z], axis=1)
        A = np.einsum('bnk,nm->bmk', proj_xyz, self.array_pos+self.pos_para)
        A = np.exp(-1j*2*np.pi/lamda*A)
    ...
    def get_A(self, doas, in_f=None, array_imperfection=False):
        ...
        if array_imperfection is True:
            A = np.einsum('bnk,nm->bmk', proj_xyz, self.array_pos + self.pos_para)  # 生成-(2pi*j*d_i/lamda)
        else:
            A = np.einsum('bnk,nm->bmk', proj_xyz, self.array_pos)
    ...
    # simulate array imperfection matrix for UCA
    def retrieve_array_imperfection_m(self, rho):
        ...
        pos_bias_x = np.array([0.0, -1, -1, -1, -1, 1, 1, 1] * (M // 8 + 1)) * 0.2
        pos_bias_y = np.array([0.0, -1, -1, -1, -1, 1, 1, 1] * (M // 8 + 1)) * 0.2
        pos_bias_z = np.zeros_like(pos_bias_x)
        pos_bias = np.stack([pos_bias_x, pos_bias_y, pos_bias_z], axis=0)
        ...
        if mc_flag:
            ...
            MC_coef = mc_para ** self.scale_distance
            # 重要important
...
if __name__ == '__main__':
    from Create_2d_k_source_dataset import Create_random_k_input_theta, Create_datasets

    dataset = UCA_dataset(8, -90, 90, 1, 2)
    snap = 100
    snr = 10
    k = 3
    ...
    dataloader = array_Dataloader(dataset, 32, True, load_style='torch', input_type='scm', output_type='doa')
