...
def Create_datasets(dataset, k, theta_set, batch_size, snap, snr, snr_set=1, shared_snr=False, **kwargs):
    ...
    if isinstance(snap, tuple) and len(snap) == 2:
        if isinstance(snr, tuple) and len(snr) == 2:
            for theta in theta_set:
                if shared_snr:
                    snr_scalar = np.random.uniform(snr[0], snr[1], size=(theta.shape[0], 1))
                    cur_snr = np.repeat(snr_scalar, k, axis=1)
                else:
                    cur_snr = np.random.uniform(snr[0], snr[1], size=(theta.shape[0], k))
                cur_snap = int(np.random.randint(snap[0], snap[1] + 1, size=(1,))[0])
                dataset.Create_DOA_data(k, theta, cur_snr, s_t_type='gauss_input', snap=cur_snap, snr_set=snr_set, **kwargs)
    else:
        if isinstance(snr, tuple) and len(snr) == 2:
            for theta in theta_set:
                if shared_snr:
                    ...
                    cur_snr = np.random.uniform(snr[0], snr[1], size=(theta.shape[0], k))
                dataset.Create_DOA_data(k, theta, cur_snr, s_t_type='gauss_input', snap=snap, snr_set=snr_set, **kwargs)
        else:
            for theta in theta_set:
