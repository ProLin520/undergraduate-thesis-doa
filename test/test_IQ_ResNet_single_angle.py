...
def evaluate_models(rho=0.0, snr_to_test=0, device=None):
    device = device or ("cuda" if torch.cuda.is_available() else "cpu")

    data_dir = rf"D:\Python\Project\doa_estimation\Graduation\data\IQ_Data\Single_Source\Test_Rho{rho}"
    ...
    vit_base_weight = rf"D:\Python\Project\doa_estimation\Graduation\result\vit\vit_M_8_k_1_base\weight_base_bestIQ_rho{rho}.pth"
    vit_transfer_weight = rf"D:\Python\Project\doa_estimation\Graduation\result\vit\vit_M_8_k_1_base_transfer\weight_transfer_bestIQ_rho{rho}.pth"
    ...
    embedding_dim = 768
    vit_models = {}
    if os.path.exists(vit_base_weight):
        model_vit_base = VisionTransformer(
        ...
        model_vit_base.load_state_dict(torch.load(vit_base_weight, map_location=device))
        model_vit_base.eval()
        vit_models["vit_base"] = model_vit_base

    if os.path.exists(vit_transfer_weight):
        model_vit_transfer = VisionTransformer(
            embed_layer=scm_embeding(8, embedding_dim),
            embed_dim=embedding_dim,
            out_dims=181,
            drop_ratio=0,
        ...
        model_vit_transfer.load_state_dict(torch.load(vit_transfer_weight, map_location=device))
        model_vit_transfer.eval()
        vit_models["vit_transfer"] = model_vit_transfer
    ...
    for target_angle in test_angles_range:
        ...
        with torch.no_grad():
            for name, model in vit_models.items():
                outputs_vit = model(x_vit_tensor)
                pred_vit = torch.argmax(outputs_vit, dim=1).cpu().numpy() - 90
                rmses[name].append(np.sqrt(np.mean((target_angle - pred_vit) ** 2)))
                append_prediction_stats(name, target_angle, pred_vit)
    ...
    save_csv(per_angle_records, BPSK_DATA_DIR / "SingleSource" / f"bpsk_single_angle_per_angle_{rho_tag}.csv")
    save_csv(summary_records, BPSK_DATA_DIR / "SingleSource" / f"bpsk_single_angle_summary_{rho_tag}.csv")
...
def plot_figure_5(angles, rmses, rho=0.0, snr_to_test=0):
    ...
    if "vit_base" in rmses:
        plt.plot(angles, rmses["vit_base"], "D", color="red", markerfacecolor="none", label="ViT", markersize=6, alpha=0.8)
    if "vit_transfer" in rmses:
        plt.plot(angles, rmses["vit_transfer"], "D", color="red", markerfacecolor="none", label="ViT Transfer", markersize=6, alpha=0.8)
    ...
    save_path = rf"D:\Python\Project\doa_estimation\Graduation\result\plot\bpsk\M_8_K_1_rho{rho}\Comparison_single_source_rho{rho}.png"
...
if __name__ == "__main__":
    rho = 0.0
    snr_to_test = 0
    angles, rmses = evaluate_models(rho=rho, snr_to_test=snr_to_test)
    plot_figure_5(angles, rmses, rho=rho, snr_to_test=snr_to_test)
