...
def build_spice_input(inputs_complex, snap):
...
def main():
    ...
    batch_size = 128
    val_batch_size = 128
    model_type = "Learning-SPICE"

    print(f"🚀 启动 Learning-SPICE SevenSource | Rho={rho}")
    ...
    save_dir = r"/result/MLP/SevenSource"
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, f"LearningSPICE_Gaussian_SevenSource_rho{rho}.pth")
    record_path = os.path.join(save_dir, f"best_LearningSPICE_SevenSource_rho{rho}_record.json")
    ...
    for epoch in range(epochs):
        ...
        print(f"\n[Epoch {epoch + 1}/{epochs}] 正在训练 Learning-SPICE SevenSource...")
        ...
        for inputs_complex, labels_onehot in tqdm(train_loader, leave=False):
            ...
            X_input = build_spice_input(inputs_complex, snap)
        ...
        if score < best_score:
            ...
            print(f"⭐ 已保存 Learning-SPICE SevenSource 最优模型 | Score={best_score:.4f} | RandAvg={best_rand:.4f}° | FixedAvg={best_fixed:.4f}° | ShiftedAvg={best_shifted:.4f}°")

    print(f"\n✅ Learning-SPICE SevenSource 训练结束，最优模型已保存至: {save_path}")
