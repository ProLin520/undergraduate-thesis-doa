...
def load_mlp_sevensource_data(base_dir, split_name):
    snrs = [0, 5, 10, 15, 20]
    x_list, y_list = [], []
...
save_folder = os.path.join(root_path, "result", "MLP", "SevenSource")
os.makedirs(save_folder, exist_ok=True)

x_train, y_train = load_mlp_sevensource_data(data_dir, "Train")
x_val, y_val = load_mlp_sevensource_data(data_dir, "Val")
...
print(f"Training seven-source Article MLP with rho={rho}...")
model.fit(x_train, y_train, epochs=50, batch_size=128, shuffle=True, validation_data=(x_val, y_val), callbacks=[lr_scheduler])

save_path = os.path.join(save_folder, f"Model_MLP_ClassifyIQ_SevenSource_Article_rho{rho}.h5")
model.save(save_path)
print(f"Saved Article MLP to: {save_path}")
