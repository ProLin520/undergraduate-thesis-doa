...
def load_seven_source_dataset(base_path):
    snrs = [0, 5, 10, 15, 20]
...
def train_and_infer_seven_source(train_loader, val_loader, rho, device="cuda"):
    ...
    print(f"Training seven-source Article IQ-ResNet with rho={rho} on {device}")
    ...
    for epoch in range(num_epochs):
        ...
        val_hit_rate = total_hits / total_targets
        epoch_loss = running_loss / len(train_loader.dataset)
        scheduler.step()
        print(f"Epoch {epoch + 1}/{num_epochs}, Loss: {epoch_loss:.4f}, Val Hit Rate: {val_hit_rate:.4f}")

        if val_hit_rate > best_val_hit_rate:
            best_val_hit_rate = val_hit_rate
            best_model_wts = copy.deepcopy(model.state_dict())

    save_path = os.path.join(root_path, "result", "IQ_ResNet", "SevenSource",
                             f"IQ_ResNet_SevenSource_Article_rho{rho}.pth")
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    model.load_state_dict(best_model_wts)
    torch.save(model.state_dict(), save_path)
    print(f"Saved best Article IQ-ResNet to: {save_path}")


if __name__ == "__main__":
    rho = 0.0
    data_base = os.path.join(root_path, "data", "IQ_Data", "Seven_Source", f"Seven_Source_Article_Rho{rho}")
    if not os.path.exists(data_base):
        print(f"Dataset path not found: {data_base}")
    else:
        train_loader, val_loader = load_seven_source_dataset(data_base)
        device = "cuda" if torch.cuda.is_available() else "cpu"
        train_and_infer_seven_source(train_loader, val_loader, rho=rho, device=device)
